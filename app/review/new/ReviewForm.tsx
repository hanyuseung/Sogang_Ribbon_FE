...
export default function ReviewForm() {
  ...
  useEffect(() => {
    if (!user) {
      const redirect = isEditMode
        ? `/review/new?reviewId=${reviewId}`
        : `/review/new?placeId=${placeId}`;
      router.replace(`/login?redirect=${redirect}`);
    }
  }, [user, router, placeId, reviewId, isEditMode]);

  // edit 모드: 기존 리뷰 데이터 fetch
  useEffect(() => {
    ...
    fetch(`/api/reviews/${reviewId}`)
      ...
      .then((data) => {
        ...
        setSelectedKeywords(data.keywords.map((rk: { keyword: { id: string } }) => rk.keyword.id));
  ...
  const canSubmit = selectedKeywords.length > 0 && content.trim().length > 0;
  ...
  const handleSubmit = async () => {
    ...
    const res = isEditMode
      ? await fetch(`/api/reviews/${reviewId}`, {
          ...
          body: JSON.stringify({ content, keywordIds: selectedKeywords }),
        })
          ...
          <div className="bg-white rounded-3xl border border-zinc-100 overflow-hidden">
            ...
            <div className="px-4 py-5">
              ...
              <textarea
                ...
                placeholder={GUIDELINES.map((line) => `• ${line}`).join("\n")}
                rows={6}
